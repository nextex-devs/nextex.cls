...
%% Beginning of file 'sample10.tex'
%%
%% Modified 2019 June
%%
%% This is a sample manuscript marked up using the
%% NexTex v1.0 LaTeX 2e macros.
%%
...
%%
%%
%% using nextex version 1.0
\documentclass[clean]{../cls/nextex10}
...
\title{Template \nextex{} Article with Examples: 
v1.0\footnote{Released on not yet!}}
...
\affiliation{American Astronomical Society, 1667 K Street NW, Suite 800, Washington, DC 20006, USA}
...
includes a history of \nextex\ and documents the new features in the
...
2.08 style file called \nextex.  These early \nextex\ versions introduced
...
\nextex\ v5.2 was released in 2005 and introduced additional graphics
...
When planning the next \nextex\ release the popularity of emulateapj played
an important roll in the decision to drop the old base code and adopt and
modify emulateapj for \nextex\ v6.+ instead.  The change brings \nextex\
inline with what the majority of authors are already using while still
delivering new and improved features.  \nextex\ v6.0 through v6.3 were
...
how to use the full potential of \nextex\ v6+.  The next section describes
...
The default style in \nextex\ v6.3 is a tight single column style, e.g.  10
...
\nextex's deluxetable environment. The deluxetable construct handles long
...
\nextex\ v6 introduced five new table features that were designed to make
...
Both the \latex\ tabular and \nextex\ deluxetable require an argument to
...
be tedious in long or complex tables.  To address this \nextex\ introduces
...
columns.   In \nextex's deluxetables the header row is specifically
...
\nextex\ has a {\tt\string\plotone} command to display a figure consisting
...
easily format.  To make the process easier for authors \nextex\ v6 offers
...
\nextex\ v6 the underlying mark up commands had to be inserted by hand
...
package.  \nextex\ requires this package so there is no need to
...
response to editor and referee comments.  In \nextex\ new commands
...
available with \nextex\ v6, will output bibtex ``@misc'' type properly.
...
\nextex v6.3. Instead of the {\tt\string\rotate} command, a new environment
...
%% the bibliography. The sample10.bib file was populated from ADS. To
%% get the citations to show in the compiled file do the following:
%%
%% pdflatex sample10.tex
%% bibtext sample10
%% pdflatex sample10.tex
%% pdflatex sample10.tex
